...
<template>
  <transition name="fade">
    <div class="fixed top-0 left-0 p-6 md:p-10 w-full min-h-screen flex justify-center z-40" :class="`items-${align}`"
      v-show="openModal">
      <transition name="opacityTransition">
        <div class="max-h-90vh bg-white rounded-md z-50 pb-2 overflow-y-auto absolute" :class="width">
          ...
          <div class="px-4 pt-2 sticky top-0 z-10 bg-white">
            <div class="w-full flex items-center justify-between border-b-px border-b-solid">
              <strong class="text-xl border-b-4 border-b-solid border-#E8A29E">{{ title }}</strong>
